...
"""
le metes una lista de 100 hr y 100 int 
"""
...
def find_peaks(x, size, min_height, min_dist, max_num):
    ...
    """
    ir_valley_locs, n_peaks = find_peaks_above_min_height(
        x, size, min_height, max_num)
    ir_valley_locs, n_peaks = remove_close_peaks(
        n_peaks, ir_valley_locs, x, min_dist)
    ...
    while i < n_peaks:
        ...
        while j < old_n_peaks:
            ...
            n_dist = (sorted_indices[j] - sorted_indices[i]
                      ) if i != -1 else (sorted_indices[j] + 1)
